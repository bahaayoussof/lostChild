...
public class AddressService {
    ...
    /**
     *  Get all the addresses where Child is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<AddressDTO> findAllWhereChildIsNull() {
        log.debug("Request to get all addresses where Child is null");
        return StreamSupport
            .stream(addressRepository.findAll().spliterator(), false)
            .filter(address -> address.getChild() == null)
            .map(addressMapper::toDto)
